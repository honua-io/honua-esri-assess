...
```bash
# Dry run (report only):
dotnet run --project packages/maui/cli -- ./MobileApp

# Apply the rewrite in place:
dotnet run --project packages/maui/cli -- ./MobileApp --write

# Annotate manual-review call sites with inline TODO comments:
dotnet run --project packages/maui/cli -- ./MobileApp --write --annotate-todos

# CI gate: fail if any manual-review markers remain:
dotnet run --project packages/maui/cli -- ./MobileApp --fail-on-manual
```
...
Build a distributable tool package from this repository with:

```bash
dotnet pack packages/maui/cli/Honua.Migrate.Maui.Cli.csproj --configuration Release
```

Install the resulting package from its configured NuGet source with:

```bash
dotnet tool install --global Honua.Migrate.Maui.Tool
```

The installed command is `honua-migrate-maui` and accepts the same arguments as
the project-run examples above.
