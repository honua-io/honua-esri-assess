"""Shared test helpers for the entitlements fixture-driven tests."""
...
class StubHttpClient:
    """Deterministic HTTP client driven by a path→response map.

    Each handler is invoked with the full URL (already redacted-friendly)
    and the query params dict, and returns an :class:`HttpResponse`. The
    client records every URL it sees so tests can assert on call order /
    redaction.
    """
...
def _match_handler(
    ...
    # Prefer the longest matching suffix so more-specific paths win.
    matches = sorted(
